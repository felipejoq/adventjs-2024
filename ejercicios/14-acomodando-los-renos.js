...
function minMovesToStables(reindeer, stables) {
  const sortedReindeer = reindeer.slice().sort((a, b) => a - b);
  const sortedStables = stables.slice().sort((a, b) => a - b);

  return sortedReindeer.reduce((totalMoves, reindeerPos, i) =>{
    return totalMoves + Math.abs(reindeerPos - sortedStables[i]);
  }, 0);
}
